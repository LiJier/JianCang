 ...
 *
 * <p>It is recommended that you save this class if it is going to be reused for better
 * performance.  This class
 * is thread-safe, but can only process a single document concurrently.</p>
 *
 * <p><strong>Usage:</strong></p>
 *
 * <p>Basic usage involves instantiating this class with a specific set of options, and calling
 * one of the
...
public class Remark {
     ...
     *
     * @param options Specified options to use on this instance.  See the docs for the Options
     *                class for common options sets.
     */
    public Remark(Options options) {
        ...
        for (final IgnoredHtmlElement el : options.getIgnoredHtmlElements()) {
            ...
            if (!el.getAttributes().isEmpty()) {
                safelist.addAttributes(el.getTagName(),
                        el.getAttributes().toArray(new String[el.getAttributes().size()]));
            }
        }
        ...
        if (options.getTables().isLeftAsHtml()) {
            ...
            // since they are automatically ignored recursively, this is the only node we worry
            // about.
     ...
     *
     * <p><strong>Note: The convert methods on the returned class will always return {@code null}
     * .</strong></p>
     *
     ...
     * <p>Example:</p>
     *
     * <blockquote>{@code new Remark(options).withOutputStream(myOut).convert(htmlText);}
     * </blockquote>
     *
     ...
     *
     * @param file    The file to load.
     * @param charset The charset of the file (if not specified and not UTF-8). Set to {@code
     * null} to determine from {@code http-equiv} meta tag, if present, or fall back to {@code
