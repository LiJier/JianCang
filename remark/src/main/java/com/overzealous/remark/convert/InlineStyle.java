...
public class InlineStyle extends AbstractNodeHandler {
    ...
    private static final Pattern ITALICS_PATTERN = Pattern.compile("font-style:\\s*italic",
            Pattern.CASE_INSENSITIVE);
    private static final Pattern BOLD_PATTERN = Pattern.compile("font-weight:\\s*bold",
            Pattern.CASE_INSENSITIVE);

    private static final Pattern INWORD_CHARACTER = Pattern.compile("\\w");

    private static final Pattern SPACE_CONTENT_SPACE = Pattern.compile("^(\\s*+)(.*?)(\\s*)$",
            Pattern.DOTALL);
    ...
    /**
     * Renders inline styling (bold, italics) for the given tag.  It handles implicit styling
     * ({@code em}, {@code strong}) as
     * well as explicit styling via the {@code style} attribute.
     * <p>This object keeps track of the depth of the styling, to prevent recursive situations
     * like this:</p>
     *
     * <blockquote>{@code <em>hello <em>world</em></em>}</blockquote>
     *
     * <p>A naive method would be render the example incorrectly (the output would be {@code
     * *hello **world*})</p>
     *
     ...
     *
     * <p>It's unique because we have to deal with leading and trailing spaces, among other
     * issues.</p>
     *
     * @param parent    The previous node walker, in case we just want to remove an element.
     * @param node      Node to handle
     ...
     */
    private void handleStyled(NodeHandler parent, Element node, DocumentConverter converter,
                              Rules rules) {
